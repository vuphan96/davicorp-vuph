                    ...
                    <div class="card-tools not-print">
                        ...
                        <div class="btn-group float-right" style="margin-right: 10px">
                            <a disabled data-perm="order_import:print" onclick="printPdf('{{ $orderImport->id }}');" href="#" class="btn btn-flat btn btn-primary" data-toggle="modal">
                                <i class="fa fa-print"></i>&nbsp;{{ sc_language_render('admin.order.print_invoice') }}
                            </a>
                        </div>
                        ...
                        <div class="btn-group float-right" style="margin-right: 10px">
                            <a data-perm="order_import:create" href="{{sc_route_admin('order_import.create')}}" class="btn btn-flat btn-primary">
                                <i class="fa fa-plus" title="Tạo mới đơn hàng nhập"></i></a>
                        </div>
                        <div class="btn-group float-right" style="margin-right: 10px">
                            ...
                            <div class="card collapsed-card">
                                <div class="table-responsive">
                                    <table id="table-product" class="table table-hover box-body text-wrap table-bordered table-product">
                                        ...
                                        @endphp
                                        @foreach ($orderImport->details->groupBy('product_id') as $keyProduct => $productGroup)
                                            <tr class="ordered-products product-parent">
                                                <td style="text-align: center">{{ $i++ }}</td>
                                                <td class="overflow_prevent">{{ $productGroup->first()->product_code }}</td>
                                                <td>{{  $productGroup->first()->product_name }}</td>
                                                <td class="product_qty">{{ $productGroup->sum('qty_order') }}</td>
                                                <td class="product_qty">{{ $productGroup->sum('qty_reality') }}</td>
                                                <td class="product_price">{{ $productGroup->first()->unit_name ?? ''}}
                                                <td class="product_price"></td>
                                                <td class="product_total">{{ sc_currency_render( round($productGroup->sum('amount') ?? 0), 'vnd') }}</td>
                                                <td class="product_comment" style="overflow: hidden"></td>
                                                <td style="text-align: center">
                                                    <span data-perm="order_import:edit_info"
                                                          onclick="showItemChild($(this),'{{ $keyProduct }}');"
                                                          class="btn btn-outline-warning btn-xs" data-title="Hiển thị chi tiết"><i
                                                                class="fas fa-eye" aria-hidden="true"></i></span>
                                                </td>
                                            </tr>
                                            @foreach($productGroup as $item)
                                            <tr class="product-child-{{$keyProduct}} d-none">
                                                <td style="text-align: center"></td>
                                                <td class="overflow_prevent"></td>
                                                <td>
                                                    @if($orderImport->status != 3)
                                                        <a type="button" onclick="showPopupChooseSupplier($(this), '{{ $item->id }}')">{{ $item->customer_name }}</a>
                                                    @else
                                                        {{ $item->customer_name }}
                                                    @endif
                                                </td>
                                                ...
                                                <td class="product_comment" style="overflow: hidden"><a
                                                            data-perm="order_import:edit_info" perm-type="disable" href="#"
                        ...
                        <div class="tableFixHead">
                            <table class="table table-hover box-body text-wrap table-bordered">
                                ...
                                <tr>
                                    ...
                                    <th style="min-width: 200px;">Sản phẩm theo khách hàng</th>
                                    <th style="width: 3.5%; text-align: right">Số lượng ban đầu</th>
                                    <th style="width: 3.5%; text-align: right">{{ sc_language_render('order.admin.return_no') }}</th>
                                    <th style="width: 4%; text-align: right">{{ sc_language_render('product.price') }}</th>
                                ...
                                @foreach ($orderImport->return as $return)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $return->product_name }}: ({{ $return->customer_name }})</td>
                                        ...
                                        @if($orderImport != 3)
                                            <td class="undo_detail"><span onclick="undoReturnOrder('{{$return->id}}', '{{$return->import_detail_id}}')" class="btn btn-sm btn-sm btn-warning"><i class="fas fa-undo text-white"></i></span></td>
                                        @endif
                                    </tr>
                                @endforeach
                                </tbody>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        function showItemChild(element, idKey)
        {
            if ($('.product-child-'+idKey).hasClass('d-none')) {
                $('.product-child-'+idKey).removeClass('d-none');
            } else {
                $('.product-child-'+idKey).addClass('d-none');
            }
        }
