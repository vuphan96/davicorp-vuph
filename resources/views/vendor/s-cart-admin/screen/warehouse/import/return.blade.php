    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card p-3">
                <div class="card-header with-border">
                    ...
                    <div class="card-tools not-print">
                        <div class="btn-group float-right" style="margin-right: 10px">
                            <a class="btn btn-outline-primary" href="{{ route('order_import.edit', ['id' => request('id')]) }}"><i
                                        class="fa fa-arrow-alt-circle-left"></i> {{ sc_language_render('action.back') }}
                            </a>
                        </div>
                        <div class="btn-group float-right" style="margin-right: 10px">
                            <a href="{{ sc_route_admin('order_import.edit', ['id' => $orderImport->id]) }}" target="_blank" class="btn btn btn-primary"><i
                ...
                <form id="form_return_import_order" action="{{ sc_route_admin('order_import.store_return', ['id' => $orderImport->id]) }}" method="post">
                    @csrf
                        ...
                        <div class="col-sm-12">
                            <div class="card collapsed-card">
                                <div class="table-responsive">
                                    <table class="table table-hover box-body text-wrap table-bordered">
                                        ...
                                        @foreach ($orderImport->details as $item)
                                            <tr>
                                                <td class="overflow_prevent">{{ $item->product_code?? 'Sản phẩm bị xoá' }}</td>
                                                <td>{{ $item->product_name }} {{ $item->customer_name != '' ? ': ('.$item->customer_name.')' : '' }}
                                                <td class="product_qty">{{ $item->qty_reality ?? 0 }}</td>
                                                <td class="product_price"><input name="qty_reality[{{ $item->id }}]"
                                                                                 id="qty_{{ $item->id }}" type="number"
                                                                                 class="form-control qty_reality"
                                                                                 oninput="returnHandle({{ $item->product_price }}, {{ $item->qty_reality }}, '{{ $item->id }}')"
                                                                                 value="0" min="0" step="any"/></td>
                                                <td class="product_price">{{ sc_currency_render($item->product_price, 'VND') }}</td>
                                ...
                                <div style="display: flex; flex-direction: row; justify-content: end" class="p-3">
                                    <button type="button" id="submitReturnOrder" class="btn btn-primary mr-3"><i
                        ...
                        <div class="tableFixHead">
                            <table class="table table-hover box-body text-wrap table-bordered">
                                ...
                                <tr>
                                    ...
                                    <th style="min-width: 200px">Tên sản phẩm theo khách hàng</th>
                                ...
                                <tbody>
                                @foreach ($orderImport->return as $item)
                                    @php
                                        $price_product = $item->product_price != null ? $item->product_price : ($item->detail->product_price ?? 0);
                                    @endphp
                                    <tr id="row_return_{{$item->id}}">
                                        <td>{{ $loop->iteration }}</td>
                                        <td style="width: 120px">{{ $item->getEditor() }}</td>
                                        <td>{{ $item->product_name }} {{ $item->customer_name != '' ? ': ('.$item->customer_name.')' : '' }}
                                        <td class="product_qty">{{ ($item->qty_original + $item->qty_return) ?? 0 }}</td>
                                        <td class="product_price">{{ $item->qty_return ?? 0 }}</td>
                                        <td class="product_price">{{ sc_currency_render($price_product ?? 0 , 'VND') }}</td>
                                        <td class="product_price">{{ sc_currency_render(($item->qty_return ?? 0) * ($price_product ?? 0), 'VND') }}</td>
                                        <td class="product_comment">{{ formatDateVn($item->created_at, true) ?? 'Trống' }}</td>
                                        <td class="undo_detail"><span onclick="undoReturnOrder('{{$item->id}}', '{{$item->import_detail_id}}')" class="btn btn-sm btn-sm btn-warning"><i class="fas fa-undo text-white"></i></span></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $('#submitReturnOrder').click(function () {
            ...
            $('.qty_reality').each(function (index) {
                let value = $(this).val();
                if (value != 0 && value != '') {
                    return flagSubmit = true;
                }
            })

            if (!flagSubmit) {
                alertJs('error', 'Vui lòng nhập số lượng trước khi lưu!');
            }
            console.log(flagSubmit);
            if (flagSubmit) {
                $('#form_return_import_order').submit();
                // setTimeout(function() {
