...
<head>
    ...
    <title>Suất ăn - {{ $order->id_name }}</title>
</head>
<body>
    <table>
        <tr>
    ...
    <table>
        ...
        <tr>
            ...
            <td colspan="1" style="font-size: 12pt"> STT: {{ $order->customer_num ?? ''}}</td>
        </tr>
        </tbody>
        <thead>
            <tr>
        ...
        <tbody>
            <tr>
                ...
                <td style="border-right: 1px solid black; text-align: right;">{{ $order->number_of_reality_servings }}</td>
                <td style="border-right: 1px solid black; text-align: right;">{{ $order->customer->serving_price ?? $order->price_of_servings }}</td>
            </tr>
            <tr>
            ...
            @php
                $sum = ($order->number_of_reality_servings)
                                * ($order->customer->serving_price ?? $order->price_of_servings);
            @endphp
            <tr>
