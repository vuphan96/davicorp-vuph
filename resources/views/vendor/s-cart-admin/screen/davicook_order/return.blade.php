                            ...
                            <div class="card collapsed-card">
                                <div class="table-responsive">
                                    <table class="table table-hover box-body text-wrap table-bordered">
                                        ...
                                        <tbody>
                                        @foreach ($productDetails->where('type', 0) as $item)
                                            <tr>
                                                <td class="dish_name">{{ $item->dish_name}}</td>
                                                <td class="overflow_prevent">{{ $item->product->sku ?? 'Sản phẩm bị xoá' }}</td>
                                                <td class="product_name_return">{{ $item->product->name ?? 'Sản phẩm bị xoá'}}
                                                <td class="product_qty">{{ $item->real_total_bom ?? 0 }}</td>
                                                <td class="product_price"><input name="qty[{{ $item->id }}]"
                                                                                 id="qty_{{ $item->id }}" type="number"
                                                                                 class="form-control"
                                                                                 oninput="returnHandle({{ $item->import_price }}, {{ $item->real_total_bom }}, '{{ $item->id }}')"
                            ...
                            <div class="card collapsed-card">
                                <div class="table-responsive">
                                    <table class="table table-hover box-body text-wrap table-bordered">
                                        ...
                                        <tbody>
                                        @foreach ($productDetails->where('type', 1) as $item)
                                            <tr>
                                                <td class="dish_name">{{ $item->dish_name}}</td>
                                                <td class="overflow_prevent">{{ $item->product->sku ?? 'Sản phẩm bị xoá' }}</td>
                                                <td class="product_name_return">{{ $item->product_name ?? 'Sản phẩm bị xoá'}}</td>
                                                <td class="product_qty">{{ $item->real_total_bom ?? 0 }}</td>
                                                <td class="product_price"><input name="qty[{{ $item->id }}]"
                                                                                 id="qty_{{ $item->id }}" type="number"
                                                                                 class="form-control"
