    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form action="{{ sc_route_admin('product_exchange.update', $masterProductExchange->id) }}" method="post" name="form_name" accept-charset="UTF-8"
                    ...
                    <div id="main-add" class="card-body">
                            ...
                            <div class="input-group">
                                <select class="form-control category select2" id="product_id" data-placeholder="Chọn sản phẩm cơ sở"
                                    ...
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}" {{ $masterProductExchange->product_id == $product->id ? 'selected' : '' }}>{{$product->name ?? ''}} - {{ $product->unit->name ?? '' }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-sm-12" id="box-product-exchange">
                            <div class="item-product-exchange">
                                ...
                                <div class="input-group">
                                    <select onchange="selectProductExchange($(this));" class="form-control product-exchange select2"
                                        ...
                                        @foreach ($products as $k => $productExchange)
                                            <option value="{{ $productExchange->id }}" {{ $masterProductExchange->product_exchange_id == $productExchange->id ? 'selected' : '' }}>{{$productExchange->name ?? ''}} - {{ $productExchange->unit->name ?? 'Lỗi' }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <label for="category" class="col-sm-12 col-form-label">
                                ...
                                <div class="input-group">
                                    <input type="number" oninput="checkValueQty($(this));" class="input-qty-exchange" min="1" value="{{ $masterProductExchange->qty_exchange }}" name="qty_exchange[]" required width="100%" placeholder="Quy đổi tương ứng">
                                </div>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        ...
        function selectProductExchange(e) {
            ...
            if (uniqueArrProductExchange.length !== arrProductExchange.length) {
                alertMsg('error', 'Sản phẩm quy đổi đã tồn tại!');
