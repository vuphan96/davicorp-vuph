...
@foreach($data as $keyCustomer => $itemCustomer)
    @foreach($itemCustomer as $keyBillDate => $itemBillDate)
        @forelse($itemBillDate as $keyExplain => $itemExplain)
            <div id="invoice" style="padding-top: {{ $j == 0 ? '0px' : '25px'}}" >
                ...
                <table>
                    ...
                    <tr>
                        <td colspan="3" class="invoice_time">
                            {{ formatStringDate(($itemExplain->first()->bill_date), 'Ngaythangnam') }} </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td style="width: 30%" class="invoice_order_common_info">
                            Tuyến hàng: {{ $itemExplain->first()->customer->route ?? ''}}
                        </td>
                ...
                <table class="table_detail">
                    ...
                    <tr>
                        ...
                        <td class="currency qty">{{ ($itemExplain->sum('number_of_reality_servings') ?? 0) }}</td>
                        <td class="currency price">{{ money_format($itemExplain->first()->customer->serving_price ?? $itemExplain->first()->price_of_servings) }}</td>
                    </tr>
                    @php
                        $sum = (($itemExplain->sum('number_of_reality_servings') ?? 0))
                            * ($itemExplain->first()->customer->serving_price ?? $itemExplain->first()->price_of_servings);
                        $j++;
                    @endphp
                    <tr>
                        <td>&nbsp;</td>
