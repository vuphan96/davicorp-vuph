...
@foreach($data as $key => $datum)
    <div id="invoice" style="padding-top: {{ $key == 0 ? '0px' : '25px' }}">
        <table>
            <tr>
        ...
        <table class="table_detail">
            ...
            @php $no1 = 1; $sum1 = 0; @endphp
            @forelse($datum->details->where('type',0)  as $keyType => $detail)
                <tr>
                    <td class="no">{{ $no1++ }}</td>
                    <td class="name">{{ $detail->product_name }}</td>
                    <td class="qty">{{ $detail->product_unit }}</td>
                    <td align="right">{{ number_format(round($detail->real_total_bom, 2) ?? 0, 2, '.', '') }}</td>
                </tr>
                @php $sum1 += round($detail->real_total_bom, 2); @endphp
            @empty
                <tr>
    ...
    @if(!empty($datum->details->where('type',1)->toArray()))
        <p style="page-break-after: always;">&nbsp;</p>
        <div id="invoice" style="margin-top: 25px;">
            <table>
                <tr>
                    <td rowspan="1" style="width: 3.5cm;padding-right: 0.5cm"><img class="logo" src="{{ asset('images/print_assets/davicook.png') }}"/>
                    </td>
            ...
            <table class="table_detail">
                ...
                @php $no2 = 1; $sum2 = 0; @endphp
                @forelse($datum->details->where('type',1)  as $keyType => $detail)
                    <tr>
                        <td class="no">{{ $no2++ }}</td>
                        <td class="name">{{ $detail->product_name }}</td>
                        <td class="qty">{{ $detail->product_unit }}</td>
                    ...
                    @php $sum2 += round($detail->real_total_bom, 2); @endphp
                @empty
                    <tr>
                        <td colspan="4">Không có dữ liệu!</td>
                    </tr>
