...
<aside class="main-sidebar sidebar-light-pink elevation-4 sidebar-no-expand">
    ...
    <a href="{{ sc_route_admin('admin.home') }}" class="brand-link navbar-secondary">
        <img src="{{ sc_file('images/davicorp_logo.png') }}" class="img-fluid" style="width: 110px; height: auto"/>
...
@if (count($menus))
    ...
    @foreach ($menus[0] as $levelTop)
        @if (!empty($menus[$levelTop->id]))
            <!-- Sidebar -->
                <div class="sidebar {{$activeMenu == $levelTop->id ? 'active' : ''}} {{ config($styleDefine.'.sidebar') }}" id="menu_{{ $levelTop->id }}">
                                                                ...
                                                                @if($level2->uri)
                                                                    <li class="nav-item {{ \Admin::checkUrlIsChild(url()->current(), sc_url_render($level2->uri)) ? 'active' : '' }}">
                                                                        <a href="{{ $level2->uri?sc_url_render($level2->uri):'#' }}"
                                                                           class="nav-link">
                                                                            <i class="{{ $level2->icon }} nav-icon pl-2"></i>
                                                                                    ...
                                                                                    @if($level3->uri)
                                                                                        <li class="nav-item {{ \Admin::checkUrlIsChild(url()->current(), sc_url_render($level3->uri)) ? 'active' : '' }}">
                                                                                            <a href="{{ $level3->uri?sc_url_render($level3->uri):'#' }}"
                                                                                               class="nav-link">
                                                                                                <i class="{{ $level3->icon }} nav-icon  pl-2"></i>
