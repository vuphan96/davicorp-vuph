    ...
    <ul class="navbar-toolbar">
        @foreach ($menus[0] as $levelTop)
            @if (!empty($menus[$levelTop->id]))
                <li id="tab_{{$levelTop->id}}" class="navbar-toolbar_item {{$activeMenu == $levelTop->id ? 'active': ''}}">
                    <a href="#" class="menuTopItem" onclick="clickTopMenu('{{$levelTop->id}}')">
                        <i class="nav-icon {{ $levelTop->icon }}" style="margin-right: 3px;"></i>
                        <p>
                            {!! sc_language_render($levelTop->title) !!}
                        </p>
                    </a>
                </li>
            @endif
