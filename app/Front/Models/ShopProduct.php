...
class ShopProduct extends Model
{
    ...
    public function qr()
    {
        return $this->hasOne(ShopQrCode::class, "link", "qr_code");
    }
    ...
    {
        return $this->hasMany(ShopProductSupplier::class, 'product_id', 'id');
    }

    public function davicookProductSuppliers()
    {
        return $this->hasMany(ShopDavicookProductSupplier::class, 'product_id', 'id');
    }

    public function prices()
    {
        return $this->hasMany(AdminProductPriceDetail::class, 'product_id', 'id');
    }
    public function warehouse(){
        return $this->hasMany(AdminWarehouseProduct::class, 'product_id','id');
    }
    public function unit()
    {
        return $this->hasOne(AdminUnit::class, 'id', 'unit_id');
    }
    ...
    */
    ...
    public function getDetail($key = null, $type = null, $storeId = null, $checkActive = 1)
    {
        ...
        if (sc_check_multi_shop_installed()) {
            ...
            $product = $this->leftJoin($tableProductStore, $tableProductStore . '.product_id', $this->getTable() . '.id');
            $product = $product->leftJoin($tableStore, $tableStore . '.id', $tableProductStore . '.store_id');
    ...
    public static function updateStock($product_id, $qty_change)
    {
        ...
        if ($item) {
            ...
            if ($product->kind == SC_PRODUCT_BUILD) {
                if (is_array($product->builds) || is_object($product->builds)) {
